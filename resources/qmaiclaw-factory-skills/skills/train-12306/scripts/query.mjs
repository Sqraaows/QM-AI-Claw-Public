...
const F = {
  ...
  gr: 21, rw: 23, rz: 24, tz: 25, wz: 26, yw: 28, yz: 29, ze: 30, zy: 31, swz: 32, dw: 33,
};
...
function durationMinutes(raw) { const [h, m] = raw.split(':').map(Number); return h * 60 + m; }
...
async function queryTickets(from, to, travelDate) {
  ...
  if (!json.data?.result) { console.error('No data returned:', JSON.stringify(json).slice(0, 500)); process.exit(1); }
  return json.data;
}
...
function parseTicket(raw, stationMap) {
  ...
  return {
    ...
    departTime: v('departTime'), arriveTime: v('arriveTime'), duration: v('duration'), canBuy: v('canBuy'), date: v('date'),
    swz: v('swz'), tz: v('tz'), zy: v('zy'), ze: v('ze'),
    gr: v('gr'), rw: v('rw'), dw: v('dw'), yw: v('yw'), rz: v('rz'), yz: v('yz'), wz: v('wz'),
  };
}
function hasSeat(val) { return val && val !== '--' && val !== '' && val !== '无'; }
...
function applyFilters(tickets) {
  ...
  if (departRange) { result = result.filter(t => { const m = parseTime(t.departTime); return m >= departRange.lo && m <= departRange.hi; }); }
  const arriveRange = parseTimeRange(values.arrive);
  if (arriveRange) { result = result.filter(t => { const m = parseTime(t.arriveTime); return m >= arriveRange.lo && m <= arriveRange.hi; }); }
  const maxDur = parseDurationLimit(values['max-duration']);
  if (maxDur) { result = result.filter(t => durationMinutes(t.duration) <= maxDur); }
  if (values.available) { result = result.filter(t => t.canBuy === 'Y'); }
  if (values.seat) {
...
<style>*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,"SF Pro Text","Helvetica Neue",sans-serif;background:#f5f5f7;color:#1d1d1f}.container{max-width:1100px;margin:0 auto;padding:40px 20px}header{text-align:center;margin-bottom:32px}h1{font-size:28px;font-weight:600;letter-spacing:-.5px}h1 .arrow{margin:0 12px;color:#86868b;font-weight:300}.meta{margin-top:8px;color:#86868b;font-size:15px}.meta span+span::before{content:"·";margin:0 8px}.filters{margin-top:6px;color:#0071e3;font-size:13px}.table-wrap{background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08)}.empty{padding:60px 20px;text-align:center;color:#86868b;font-size:15px}table{width:100%;border-collapse:collapse;font-size:14px}thead{background:#fafafa}th{padding:12px 10px;font-weight:500;color:#86868b;font-size:12px;letter-spacing:.5px;border-bottom:1px solid #f0f0f0;white-space:nowrap}td{padding:11px 10px;border-bottom:1px solid #f5f5f5;text-align:center;white-space:nowrap}tr:last-child td{border-bottom:none}tr:hover{background:#fafbff}.train-code{font-weight:600;text-align:left;padding-left:16px}.type-g{color:#0071e3}.type-d{color:#34c759}.type-z{color:#af52de}.type-t{color:#ff9500}.type-k{color:#86868b}.time{font-variant-numeric:tabular-nums}.depart{font-weight:600}.arrow{margin:0 4px;color:#c0c0c0}.arrive{color:#6e6e73}.duration{color:#86868b;font-variant-numeric:tabular-nums}.na{color:#d2d2d7}.available{color:#34c759;font-weight:500}.sold-out{color:#ff3b30}.count{font-weight:600;font-variant-numeric:tabular-nums}.buy-yes{color:#34c759;font-weight:500}.buy-no{color:#ff3b30;font-weight:500}footer{text-align:center;margin-top:24px;color:#c0c0c0;font-size:12px}</style>
</head>
<body>
<div class="container">
<header><h1>${fn}<span class="arrow">→</span>${tn}</h1>
<div class="meta"><span>${e(travelDate)}</span><span>${tickets.length} 趟列车</span></div>${filterTag}</header>
<div class="table-wrap">${tickets.length === 0 ? '\n <div class="empty">没有找到符合条件的列车</div>' : `
<table><thead><tr><th style="text-align:left;padding-left:16px">车次</th><th>时间</th><th>耗时</th><th>商务/特等</th><th>一等座</th><th>二等座</th><th>软卧/动卧</th><th>硬卧</th><th>硬座</th><th>无座</th><th>状态</th></tr></thead><tbody>\n${rows}\n</tbody></table>`}</div>
...
function buildMarkdown(tickets, from, to, travelDate, filterDesc) {
  ...
  lines.push('');
  if (tickets.length === 0) { lines.push('没有找到符合条件的列车'); return lines.join('\n'); }
